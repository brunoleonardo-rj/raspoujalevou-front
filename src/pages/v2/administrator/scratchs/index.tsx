...
export default function ScratchCardsPage() {
  ...
  const fetchScratchCards = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.raspoujalevou.com/v1/api/scratchcards/admin/all?includeInactive=false', {
        method: 'GET',
        headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      const response = await fetch(`https://api.raspoujalevou.com/v1/api/scratchcards/admin/${cardToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const confirmToggleFeatured = async () => {
    ...
    try {
      setFeaturing(true);
      const response = await fetch('https://api.raspoujalevou.com/v1/api/admin/scratchcards/toggle-featured', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
